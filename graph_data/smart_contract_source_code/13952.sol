    ...
    UsingAdmin
{
    ...
    function cashOut(uint _amt) public {
        _uncreditUser(msg.sender, _amt);
    }

     
    function betWithCredits(uint64 _bet) public {
    ...
    function betFromGame(uint32 _id, bytes32 _hashCheck) public {
    ...
    function finalize(uint32 _id, bytes32 _hashCheck) public returns (bool _didFinalize) {
        Game storage _game = games[_id];
        address _user = userAddresses[_game.userId];
        if (_game.iBlock == 0)
